  ...
  return (
    <article className={featured ? "border border-cyan-300/20 bg-cyan-300/[0.035] p-5 sm:p-6" : "border border-white/8 bg-white/[0.018] p-5"}>
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-t border-white/8 pt-4">
        {employee ? <div className="flex min-w-0 items-center gap-2"><EmployeeAvatar alt={`${employee.nameKo} 프로필`} className="size-7 rounded-full" size={28} src={employee.profileImage} /><div className="min-w-0"><p className="truncate text-[11px] text-zinc-300">{employee.nameKo}</p><p className="truncate text-[9px] text-zinc-600">{team?.nameKo} · {division?.nameKo}</p></div></div> : <p className="truncate text-[10px] text-zinc-600">{team?.nameKo ?? division?.nameKo ?? "PERSOS 운영"}</p>}
