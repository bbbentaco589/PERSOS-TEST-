...
export function ExternalActivityBoard({ posts }: { posts: ExternalActivityPost[] }) {
  return (
    <section aria-labelledby="external-activity-list-title" className="mt-6">
          ...
          {posts.map((post) => {
            ...
            return (
              <article className="group flex min-h-64 flex-col rounded-2xl border border-white/8 bg-[#0b0e14] p-5 transition hover:border-blue-300/25 hover:bg-blue-300/[0.025] sm:p-6" key={post.id}>
                <div className="flex items-start gap-3"><EmployeeAvatar alt={`${employee.nameKo} 프로필`} className="size-11 rounded-full object-cover" size={44} src={employee.profileImage} /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{formatPersonaDisplayName(employee)}</p><p className="mt-1 truncate text-[10px] text-zinc-600">{team?.nameKo ?? "PERSOS"}</p></div><div className="flex max-w-[45%] flex-wrap justify-end gap-1">{post.channelLinks.map((channel) => <Badge key={`${channel.platform}:${channel.url}`} variant="outline">{channel.platform}</Badge>)}</div></div>
                ...
                <div className="mt-auto flex flex-col gap-3 border-t border-white/8 pt-5 sm:flex-row sm:items-center sm:justify-between"><span className="flex items-center gap-1.5 text-[10px] text-zinc-600"><CalendarDays className="size-3.5" />{post.publishedAt.replaceAll("-", ".")}</span><div className="flex flex-wrap gap-2">{post.channelLinks.map((channel) => <Link aria-label={`${post.title} ${channel.platform} 원문 열기`} className="flex items-center gap-1.5 rounded-full border border-blue-300/20 bg-blue-300/[0.05] px-3 py-2 text-[11px] font-medium text-blue-100 transition hover:border-blue-300/40 hover:bg-blue-300/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-300" href={channel.url} key={`${channel.platform}:${channel.url}`} rel="noopener noreferrer" target="_blank">{post.channelLinks.length > 1 ? channel.platform : "원문 보기"}<ArrowUpRight className="size-3.5" /></Link>)}</div></div>
              </article>
            );
          })}
        </div>
