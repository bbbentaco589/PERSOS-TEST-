    ...
    <div className="grid gap-3 md:grid-cols-2">
      {items.map((item) => (
        <Link
          ...
          href={item.href}
          key={item.id}
          rel={item.external && !item.boardShortcut ? "noreferrer" : undefined}
          target={item.external && !item.boardShortcut ? "_blank" : undefined}
        >
          ...
          <div className="mt-auto flex items-center justify-between gap-3 pt-5 text-[10px] text-zinc-600">
            {item.publishedAt ? <time dateTime={item.publishedAt}>{formatDate(item.publishedAt)}</time> : <span>게시판 바로가기</span>}
            <span className="grid size-8 place-items-center rounded-full border border-white/10 text-zinc-500 transition group-hover:border-cyan-300/30 group-hover:text-cyan-100">
              {item.external && !item.boardShortcut ? <ExternalLink className="size-3.5" /> : <ArrowRight className="size-3.5" />}
            </span>
          </div>
        </Link>
      ))}
