          ...
          <div className="mt-4 space-y-3">
            {posts.length === 0 ? <p className="rounded-md border border-dashed border-white/10 p-6 text-center text-xs text-zinc-600">등록된 외부 활동이 없습니다.</p> : posts.map((post) => (
              <article className="grid gap-3 rounded-md border border-white/8 bg-black/20 p-4 sm:grid-cols-[minmax(0,1fr)_auto]" key={post.id}>
                <div className="min-w-0"><div className="flex flex-wrap gap-2"><Badge variant="outline">{post.platform}</Badge><Badge variant={post.active ? "accent" : "outline"}>{post.active ? "공개" : "숨김"}</Badge></div><h3 className="mt-2 line-clamp-1 text-xs font-semibold">{post.title}</h3><p className="mt-2 flex items-center gap-1 text-[9px] text-zinc-600"><CalendarDays className="size-3" />{post.publishedAt}</p></div>
