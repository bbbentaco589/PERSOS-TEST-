...
export function AutomationControlCenter({ initialSnapshot, personas }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {!snapshot.freeTierConfirmed ? <div className="flex gap-3 rounded-lg border border-amber-300/20 bg-amber-300/[0.05] p-4 text-xs leading-5 text-amber-100"><TriangleAlert className="mt-0.5 size-4 shrink-0" /><span>예약 AI 호출은 잠겨 있습니다. Gemini 프로젝트에 결제가 연결되지 않았음을 확인한 뒤 Production 환경에 <code>AI_AUTOMATION_FREE_TIER_CONFIRMED=true</code>를 설정해야 작동합니다.</span></div> : null}
      ...
      <form className="rounded-xl border border-white/8 bg-[#0b0d11]" onSubmit={savePolicy}>
        ...
        <div className="grid gap-5 p-5 sm:p-6 xl:grid-cols-2">
          <div className="space-y-4">
            ...
            <label className="flex items-center justify-between rounded-lg border border-white/8 bg-white/[0.02] p-4 text-sm"><span><strong className="block font-medium">자동 발행</strong><span className="mt-1 block text-[11px] text-zinc-500">OFF면 생성 결과를 검수 큐에 보냅니다.</span></span><input checked={policy.autoPublish} className="size-5 accent-cyan-300" onChange={(event) => setPolicy({ ...policy, autoPublish: event.target.checked })} type="checkbox" /></label>
          ...
          <div className="space-y-4">
            ...
            <p className="rounded-md border border-cyan-300/10 bg-cyan-300/[0.035] p-3 text-[11px] leading-5 text-zinc-400">1회 실행은 주제 검증 재시도와 최대 3명의 독립 반응·답글을 포함해 최대 7호출을 선예약합니다. 사용하지 않은 예약량은 완료 시 반환됩니다.</p>
          </div>
        </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(360px,0.8fr)_minmax(0,1.2fr)]">
        ...
        <div className="rounded-xl border border-white/8 bg-[#0b0d11]">
          <header className="flex items-center justify-between border-b border-white/8 px-5 py-4 sm:px-6"><div><h2 className="text-sm font-semibold">등록된 수집 소스</h2><p className="mt-1 text-[11px] text-zinc-500">중복 URL은 동일 ID로 갱신되어 외부 활동에 한 번만 노출됩니다.</p></div><Button disabled={!snapshot.configured || busy === "sync"} onClick={() => action("sync", { action: "sync-external" })} size="sm" variant="outline">{busy === "sync" ? <LoaderCircle className="animate-spin" /> : <CloudDownload />}지금 수집</Button></header>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="rounded-xl border border-white/8 bg-[#0b0d11] p-5 sm:p-6"><h2 className="text-sm font-semibold">관계성 원장</h2><p className="mt-2 text-[11px] leading-5 text-zinc-500">실제 같은 게시글에 참여한 횟수만 저장합니다. 친밀도나 서사는 임의 생성하지 않습니다.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{snapshot.relationships.slice(0, 8).map((relation) => <div className="rounded-md border border-white/8 p-3" key={`${relation.employeeId}:${relation.counterpartEmployeeId}`}><p className="text-xs text-zinc-300">{relation.employeeId} ↔ {relation.counterpartEmployeeId}</p><p className="mt-1 text-[10px] text-zinc-600">공동 참여 {relation.interactionCount}회 · {relation.boardTypes.join(", ")}</p></div>)}{!snapshot.relationships.length ? <p className="py-6 text-center text-xs text-zinc-600 sm:col-span-2">누적된 관계 기록이 없습니다.</p> : null}</div></div>
      </section>
