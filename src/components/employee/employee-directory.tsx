  ...
  return (
    <>
      <section aria-label="페르소나 검색 및 필터" className="border-y border-white/8 py-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-[minmax(240px,1.2fr)_180px_180px_140px_180px]">
          <label className="relative"><span className="sr-only">직원 검색</span><Search className="pointer-events-none absolute left-3 top-3 size-4 text-zinc-600" /><input className="h-10 w-full rounded-md border border-white/10 bg-[#0d1015] pl-9 pr-3 text-sm outline-none placeholder:text-zinc-700 focus:border-cyan-300/50" onChange={(event) => setQuery(event.target.value)} placeholder="이름, 직무, 전문 분야 검색" value={query} /></label>
          ...
          <select aria-label="상태" className={selectClass} onChange={(event) => setStatus(event.target.value)} value={status}><option value="all">전체 상태</option><option value="Active">운영 중</option><option value="Draft">러프</option></select>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-zinc-500"><span>공개 프로필 {filteredCharacters.length}명 / 전체 {publicCharacters.length}명</span><div className="flex gap-2"><Badge variant="accent">승인 {publicCharacters.filter((item) => item.profileStage === "Approved").length}</Badge><Badge variant="outline">Rough {publicCharacters.filter((item) => item.profileStage === "Rough").length}</Badge><span className="grid size-6 place-items-center rounded border border-white/10" title="그리드 보기"><LayoutGrid className="size-3" /></span></div></div>
      {filteredCharacters.length ? <section aria-label="AI 직원 목록" className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">{filteredCharacters.map((character) => <CharacterCard character={character} key={character.id} />)}</section> : <EmptyState title="조건에 맞는 페르소나가 없습니다" description="검색어 또는 조직·상태 필터를 조정해 주세요." />}
    </>
  );
}
