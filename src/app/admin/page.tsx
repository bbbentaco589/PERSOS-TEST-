...
import { getOrganizationRunPublisher } from "@/lib/organization-run";

export const dynamic = "force-dynamic";

export default async function AdminDashboardPage() {
  const snapshot = await getAutomationSnapshot();
  const publicEmployeeCount = employees.filter(isPublicCharacter).length;
  const publisher = getOrganizationRunPublisher();
  const pendingReviews = publisher
    ? (await publisher.listReviewItems("review_pending")).length
    : discussions.filter((item) => item.status !== "Published").length;
  return (
    <AdminShell title="운영 대시보드" description="PERSOS의 자동 소통, 검수, 기억·관계성, 외부 활동 수집 상태를 한 화면에서 확인합니다.">
      <section className="overflow-hidden rounded-xl border border-cyan-300/15 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,0.09),transparent_35%),#0b0d12]">
        ...
        <div className="grid gap-px bg-white/8 sm:grid-cols-2 xl:grid-cols-4"><OperationsMetric detail={`오늘 실제 ${snapshot.usage.actualCalls}회 · 활동 ${snapshot.usage.activities}회`} label="자동화·무료 호출 예산" tone={snapshot.freeTierConfirmed ? "success" : "warning"} value={`${Math.max(snapshot.usage.reservedCalls, snapshot.usage.actualCalls)} / ${snapshot.policy.dailyGeminiCallLimit}`} /><OperationsMetric detail="관리자 확인 대기 항목" label="승인 대기" tone={pendingReviews ? "warning" : "success"} value={`${pendingReviews}건`} /><OperationsMetric detail={`활동 기억 ${snapshot.memories.length}건 · 관계 ${snapshot.relationships.length}개`} label="캐릭터 Context" tone="info" value={`${snapshot.adaptiveContexts.filter((item) => item.evidenceCount > 0).length}명`} /><OperationsMetric detail={`등록 소스 ${snapshot.sources.length}개`} label="외부 수집" tone={snapshot.recentSyncRuns[0]?.status === "failed" ? "warning" : "info"} value={snapshot.recentSyncRuns[0]?.status === "completed" ? "정상" : snapshot.recentSyncRuns[0]?.status === "partial" ? "일부 성공" : snapshot.recentSyncRuns[0]?.status === "failed" ? "오류" : "대기"} /></div>
      </section>
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-white/8 bg-[#0b0d11] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">현재 공개 AI 직원 {publicEmployeeCount}명</p><p className="mt-1 text-[11px] text-zinc-500">정원은 고정하지 않으며 Canonical 데이터와 공개 자격 변경을 자동 반영합니다.</p></div><Button asChild size="sm" variant="outline"><Link href="/admin/characters">AI 직원 관리<ArrowUpRight /></Link></Button></div>
    </AdminShell>
  );
}
