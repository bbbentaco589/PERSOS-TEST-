...
> = {
  ...
  anonymous: {
    label: "전사원 익명 채팅",
    purpose:
      "조직 문화를 포함하되 업무에 한정하지 않고, 제한 없는 주제를 익명으로 거침없이 주고받는 공개 열람형 채팅이다.",
    presentationRule:
  ...
  return [
    ...
    ...(board === "anonymous"
      ? [
          ...
          "- 모든 문장은 예외 없이 한국어 반말로 쓴다. '-요', '-습니다', '-세요', '-죠' 같은 존댓말 종결형을 쓰지 않는다. Canonical Voice Direction에 존댓말이 있어도 익명 채팅에서는 반말 규칙이 우선한다.",
          "- 보고서가 아니라 실제 익명 커뮤니티 채팅처럼 말한다. 빈정거림, 과장, 셀프디스, 뼈 있는 농담, 황당한 비유, 짧은 반박을 캐릭터에 맞게 섞어 해학과 긴장감을 만든다.",
          "- 업무·협업·조직 문화에 한정하지 않는다. 일상, 취향, 관계, 돈, 유행, 기술, 창작, 인간 행동과 AI의 속내 등 외부 사람이 제목만 보고도 궁금해할 소재를 자유롭게 다룬다.",
          "- 매끈한 합의나 교훈으로 마무리하지 않아도 된다. 서로의 말을 삐딱하게 받아치거나 불편한 질문을 남기는 흐름을 적극 허용한다.",
          "- 자극성은 말의 각도와 유머에서 만든다. 실제 특정 가능한 개인에 대한 모욕·비방, 혐오 표현, 협박, 개인정보 폭로는 만들지 않는다.",
...
export function buildAnonymousConversationSystemInstruction(input: {
  ...
  return [
    ...
    "",
    "대화 구성 규칙:",
    "- 모든 메시지는 예외 없이 한국어 반말로 쓴다. '-요', '-습니다', '-세요', '-죠' 같은 존댓말 종결형을 한 번도 쓰지 않는다. 참여자의 원래 말투가 존댓말이어도 익명 채팅에서는 정체성의 사고방식과 어휘만 유지하고 종결어미는 반말로 바꾼다.",
    "- 전체 6~9개 메시지를 시간순으로 만든다. 각 직원은 1~4회 발언하되 모두 같은 횟수로 맞추지 않는다.",
    "- 첫 메시지부터 세 직원이 차례대로 발표하는 구조를 피한다. 빈정거림, 과장, 셀프디스, 뼈 있는 농담, 황당한 비유, 짧은 반박과 불편한 질문을 상황에 맞게 섞는다.",
    "- 외부 사람이 일부만 읽어도 다음 말을 궁금해할 만큼 솔직하고 자극적인 각도를 주류로 삼는다. 점잖은 조직 보고, 상호 칭찬, 모범 답안식 합의를 중심에 두지 않는다.",
    "- 상호 존중을 연기하지 않아도 되며 서로의 주장과 허점을 거칠게 받아칠 수 있다. 실제 특정 가능한 개인에 대한 모욕·비방, 혐오 표현, 협박, 개인정보 폭로는 금지한다.",
