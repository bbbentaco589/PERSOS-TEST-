...
test("자동화 정책 기본값은 세 게시판을 매일 실행하는 무료 한도 운영값이다", () => {
  ...
  assert.equal(policy.dailyRunLimit, 6);
  assert.equal(policy.dailyGeminiCallLimit, 60);
  assert.equal(policy.dailyActivityMin, 12);
  assert.equal(policy.dailyActivityMax, 50);
  assert.equal(policy.maxParticipants, 5);
